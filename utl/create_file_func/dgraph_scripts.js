function parseDgraphNodes(tables) {
  const foreignKeys = {};
  let primaryKey = [];
  ...
  function parseSQLTable(table) {
    ...
    createTablesCode += `# Node ==> ${table.type}`;

    // create code for each field
    for (const fieldId in table.fields) {
      createTablesCode += `\n`;
      createTablesCode += createTableField(table.fields[fieldId],table.type);
  ...
  function createTableField(field, label) {
    console.log(field, label,"field, label")
    let fieldCode = ``;
    fieldCode += `${tab} ${checkUnique(field.unique,label)}${field.name}: ${tab}${checkDataType(field.type)}`;
   ...
   // fieldCode += checkUnique(field.unique);
    ...
    let fieldCodefinal = `${fieldCode} .`;
    return fieldCodefinal;
  }
  ...
  function checkUnique(fieldUnique, label) {
    if (fieldUnique) return `${label}.`;
    return '';
  }
  ...
  function checkDefault(fieldDefault, dataType) {
    if (fieldDefault.length > 0) {
      let defaultString = `${tab}@index`;
     // if (dataType === 'String') defaultString += `(${fieldDefault})`;
      defaultString += `(${fieldDefault})`;
...
module.exports = parseDgraphNodes;
